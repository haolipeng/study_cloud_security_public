...
在敲代码给读者展示的过程中，你会发现存在两个矛盾点：

1、全程都敲代码的话，对于技术分享者的压力比较大，所以
...
关于dpdk技术的学习路径，刚才去厕所时，有了新的想法。

在AI时代，学习方式也要有所升级，从自己摸索学习 升级 为个人努力 + 导师辅助的学习方式。以dpdk hash表为例，分为三个步骤来走：

**第一步：自己学**

1）看官网文档（初步了解）

通过官网文档先了解个大概：https://doc.dpdk.org/guides-25.07/prog_guide/hash_lib.html

如果英语水平一般，则可以找一个中文文档来学（不推荐，因为知识最好学习一手的）



2）看example例子（梳理大致逻辑）

在dpdk源代码目录中有dpdk_source/app/test/test_hash.c文件，通过例子，总结出hash库的常用api（只学最常用的几个api即可玩转起来）



3）梳理出流程文档（最好是伪代码）

梳理出大致流程的伪代码，每一步分别调用哪些api，有些api的参数比较复杂的，可以查找文档大致看一下，但是不必深究API的使用，因为AI在API使用这快比我们更擅长，也更厉害。



**第二步：AI辅助学**

AI可以帮我生成一份hash库的使用demo，减少我们自己写demo的烦恼，但是我们要对照下，AI写的demo中的逻辑流程和我们之前梳理的代码流程是否一致。

如果一致：验证我们总结的知识是没问题的；

如果不一致：哪里不一致，是我理解错了？还是AI理解错了？

**需要运行demo示例代码来进行一步步验证。**



**第三步：复盘**

复盘下自己理解错误的地方，在下次同样问题的时候才不至于犯错误。
