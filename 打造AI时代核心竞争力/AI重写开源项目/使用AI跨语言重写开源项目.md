...
2、将它重写为golang项目的设计文档：生成相关的数据结构、结构体和接口的文档。另外，用 Mermaid 画出设计图表。务必使用最恰当的和最新版本的相关库
```



作为rust和golang语言的专家，你也熟悉ebpf技术和编程实战，你检查下所有的rust类型的文件，然后学习agentsight的架构设计。然后基于你的学习，在./specs目录下生成两种文件
1、产品需求文档：列出agentsight的所有功能列表，包括它的详细说明和使用示例。
2、将它重写为golang项目的设计文档：生成相关的数据结构、结构体和接口的文档。另外，用 Mermaid 画出设计图表。务必使用最恰当的和最新版本的第三方库

如果有彼此独立的任务，可自行选择开启多subagent执行任务。
